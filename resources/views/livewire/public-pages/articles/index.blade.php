    ...
    <div class="container mx-auto px-6 -mt-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 flex flex-col md:flex-row gap-4 items-center justify-between mb-12 border border-gray-100 dark:border-gray-700 relative z-10 transition-colors">
            ...
            <div class="flex overflow-x-auto pb-2 md:pb-0 gap-2 w-full md:w-auto hide-scrollbar">
                <button 
                    wire:click="setCategory(0)"
                    class="px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition {{ $categoryId === 0 ? 'bg-primary-600 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200' }}"
                >
                    Semua
                </button>
                ...
                @foreach($categories as $cat)
                    <button 
                        ...
                        class="px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition {{ $categoryId === $cat->id ? 'bg-primary-600 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200' }}"
                    >
                        {{ $cat->name }}
                    </button>
                @endforeach
        ...
        <div class="mb-12 group relative rounded-3xl overflow-hidden shadow-xl aspect-[16/7] md:aspect-[21/9]">
            <img src="{{ $featured->thumbnail }}" class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition duration-700">
            ...
            <div class="absolute bottom-0 left-0 p-6 md:p-12 max-w-3xl">
                ...
                <h2 class="text-2xl md:text-4xl font-bold text-white mb-4 leading-tight group-hover:text-secondary-400 transition">
                    <a href="{{ route('articles.detail', $featured->slug) }}">
                        {{ $featured->title }}
                    </a>
                </h2>
                ...
                <div class="flex items-center gap-4 text-sm text-gray-400">
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 rounded-full bg-primary-600 flex items-center justify-center text-white font-bold text-xs">
                            {{ substr($featured->author->name ?? 'Admin', 0, 1) }}
                        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <article class="flex flex-col bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition duration-300 group">
                <a href="{{ route('articles.detail', $article->slug) }}" class="aspect-video bg-gray-100 dark:bg-gray-700 relative overflow-hidden">
                    <img src="{{ $article->thumbnail }}" alt="{{ $article->title }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                ...
                <div class="p-6 flex flex-col flex-1">
                    ...
                    <div class="pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                        <span class="text-xs font-semibold text-gray-600 dark:text-gray-300">Oleh {{ $article->author->name ?? 'Admin' }}</span>
                        <a href="{{ route('articles.detail', $article->slug) }}" class="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-semibold text-sm flex items-center gap-1">
                            Baca Selengkapnya
